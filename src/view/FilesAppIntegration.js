...
export default {
	...
	_restoreDAV(version) {
		const restoreUrl = this.linkToRemoteBase('dav') + '/versions/' + getCurrentUser().uid
			+ '/versions/' + this.fileId + '/' + version
		$.ajax({
			type: 'MOVE',
			url: restoreUrl,
			headers: {
				Destination: this.linkToRemoteBase('dav') + '/versions/' + getCurrentUser().uid + '/restore/target',
			},
